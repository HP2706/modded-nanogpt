...
#implementation heavily borrowed from official implementation: https://github.com/facebookresearch/memory/blob/7f85280cee4a4696f5e0444d5657dc09282408ff/lingua/product_key/xformer_embeddingbag.py
...
def embedding_bag_fn(
    indices: Float[torch.Tensor, "batch indices"],
    weight: Float[torch.Tensor, "keys dim"],
    per_sample_weights: Float[torch.Tensor, "batch indices"],
    mode: str = "sum",
...
class MemoryAtScaleMLP(torch.nn.Module):
    def __init__(
        self,
        d_model: int,
        n_experts: int,        
        ...
        self.weight = torch.nn.Parameter(torch.randn(n_experts, d_model))
    ...
    def forward(
        ...
        return embedding_bag_fn(
            indices=indices, 
            weight=self.weight, 
            per_sample_weights=per_sample_weights, 
            mode="sum", 
            bw_algo="reverse_indices"
        )
